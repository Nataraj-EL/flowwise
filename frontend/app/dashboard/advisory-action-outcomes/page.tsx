...
import {
  fetchAdvisoryActionOutcomeSummary,
  evaluateAdvisoryActionOutcome,
  BackendAdvisoryActionOutcomeSummaryDTO,
  BackendAdvisoryActionOutcomeDTO,
  BackendAdvisoryActionLearningDTO,
} from '@/lib/api';

export default function AdvisoryActionOutcomesPage() {
  const [merchantId] = useState<number>(1);
  const [window, setWindow] = useState<string>('30D');
  const [loading, setLoading] = useState<boolean>(true);
  const [evaluating, setEvaluating] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [summary, setSummary] = useState<BackendAdvisoryActionOutcomeSummaryDTO | null>(null);
  ...
  const loadData = async (selectedWindow: string) => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchAdvisoryActionOutcomeSummary(merchantId, selectedWindow);
      setSummary(data);
    } catch (err: any) {
      console.error(err);
      setError(err.message || 'Failed to load advisory action outcome summary');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEvaluate = async (planId: number, stepId: number) => {
    try {
      setEvaluating(true);
      setActionNotice(null);
      await evaluateAdvisoryActionOutcome(merchantId, planId, stepId, window);
      setActionNotice(`Evaluated action step outcome for step #${stepId} (${window} window).`);
      loadData(window);
    } catch (err: any) {
      console.error(err);
      setError(err.message || 'Failed to evaluate action outcome');
    } finally {
      setEvaluating(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-[#0A0D14] text-slate-100 p-8 flex items-center justify-center">
        <div className="flex items-center gap-3 text-emerald-400">
          ...
          <span className="font-medium text-lg">Measuring Advisory Action Outcomes & Adaptive Learning...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0A0D14] text-slate-100 p-6 md:p-8 space-y-8 font-sans">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white">
                  Action Outcomes & Adaptive Learning
                </h1>
              ...
              <p className="text-slate-400 text-sm mt-1">
                Measures actual vs expected score, cash recovery & risk reduction for completed action steps to generate bounded learning multipliers.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-4 flex items-start gap-3 text-emerald-300 text-sm">
        ...
        <div>
          ...
          Outcome measurements are strictly read-only, immutable historical records (`OBSERVED_ACTION_OUTCOME`). Simulated what-if estimates are never included in actual outcome or learning calculations.
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-[#121622] border border-slate-800 p-5 rounded-2xl col-span-2 md:col-span-1 flex flex-col justify-between">
          ...
          <div className="text-[11px] text-slate-500">Across {summary?.totalEvaluatedOutcomesCount || 0} Step Outcomes ({window})</div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Evaluated Action Step Outcomes Timeline (2 cols) */}
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-lg font-semibold text-white flex items-center gap-2">
            <Layers className="w-5 h-5 text-emerald-400" />
            Measured Action Step Outcomes Timeline ({window})
          </h3>

          <div className="space-y-4">
            {summary?.outcomes?.map((o: BackendAdvisoryActionOutcomeDTO) => (
              <div
                key={o.id}
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
                  <div className="space-y-1">
                    ...
                    <h4 className="font-bold text-white text-base">Action Step #{o.stepId} Evaluation</h4>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-right">
                    ...
                    <button
                      onClick={() => handleEvaluate(o.planId, o.stepId)}
                      disabled={evaluating}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  ...
                  <div className="bg-[#0A0D14] p-3 rounded-lg border border-slate-800/80">
                    ...
                    <div className="font-bold text-slate-100">
                      Exp: ₹{o.financialImpactExpected?.toLocaleString()} | <span className="text-emerald-400">Act: ₹{o.financialImpactActual?.toLocaleString()}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#0A0D14] p-3 rounded-lg border border-slate-800/80">
                    ...
                    <div className="font-bold text-slate-100">
                      Exp: {o.riskReductionExpected?.toFixed(2)} | <span className="text-purple-400">Act: {o.riskReductionActual?.toFixed(2)}</span>
                    </div>
                  </div>

                  <div className="bg-[#0A0D14] p-3 rounded-lg border border-slate-800/80">
                    <span className="text-slate-400 block mb-1 font-medium">Confidence Status</span>
                    <div className="font-bold text-cyan-400">{o.confidenceStatus}</div>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#121622] border border-slate-800 rounded-xl p-6 space-y-4 shadow-xl">
            <h3 className="text-base font-semibold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              Action Learning Multipliers
            </h3>
            <p className="text-xs text-slate-400 leading-relaxed">
              Learned from historical observed action outcomes. Applied to future action sequencing step scores strictly bounded to <span className="text-emerald-400 font-mono">[0.900, 1.100]</span>.
            </p>

            <div className="space-y-3 pt-2">
              {summary?.learnings?.map((l: BackendAdvisoryActionLearningDTO) => (
                <div key={l.id} className="bg-[#0A0D14] p-4 rounded-xl border border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-200 text-xs">{l.actionType}</span>
